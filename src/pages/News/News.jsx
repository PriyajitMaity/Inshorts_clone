...
const News = () => {
  ...
  const apiCall = async () => {
    ...
    try {
      result = await axios.get(
        `https://gnews.io/api/v4/top-headlines?category=${category}&page=${pageNum}&lang=${language}&country=${
          params.category == "national" ? "in" : "any"
        }&max=10&apikey=${apiKey}`
      );
  ...
  useEffect(() => {
    ...
    if (params.category == undefined || !validQuaries.includes(params.category)) {
      ...
      apiCall();
      document.title =
        (params.category == "general" ? "TOP HEADLINES" : params.category.toLocaleUpperCase()) +
        " NEWS || INSHORTS CLONE";
      setCurrPath(params.category);
    }
    window.scrollTo(0, 0);
