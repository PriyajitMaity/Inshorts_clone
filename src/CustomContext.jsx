...
const MyContextProvider = (props) => {
    ...
    const [hindiBookmarkArticles, setHindiBookmarkArticlesHandler] = useState([]);
const [englishBookmarkArticles, setEnglishBookmarkArticlesHandler] = useState([]);

useEffect(() => {
  const storedHindiBookmarks = localStorage.getItem('bookmarksHindi');
  if (storedHindiBookmarks) {
    try {
      setHindiBookmarkArticlesHandler(JSON.parse(storedHindiBookmarks));
    } catch (error) {
      console.error('Error parsing Hindi bookmarks from localStorage:', error);
    }
  }

  const storedEnglishBookmarks = localStorage.getItem('bookmarksEnglish');
  if (storedEnglishBookmarks) {
    try {
      setEnglishBookmarkArticlesHandler(JSON.parse(storedEnglishBookmarks));
    } catch (error) {
      console.error('Error parsing English bookmarks from localStorage:', error);
    }
  }
}, []);
